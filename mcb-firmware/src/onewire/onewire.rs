pub use crate::onewire::address::Address;
pub use crate::onewire::commands;
pub use crate::onewire::crc;
pub use crate::onewire::error::{OneWireError, OneWireResult};
use embedded_hal::{
    delay::DelayNs,
    ...
    T: OutputPin<Error = E>,
{
    ...
    pub fn release_bus(&mut self) -> OneWireResult<(), E> {
        self.pin.set_high().map_err(|err| OneWireError::PinError(err))
    }

    /// Drives the bus low
    pub fn set_bus_low(&mut self) -> OneWireResult<(), E> {
        self.pin.set_low().map_err(|err| OneWireError::PinError(err))
    }

    pub fn is_bus_high(&mut self) -> OneWireResult<bool, E> {
        self.pin.is_high().map_err(|err| OneWireError::PinError(err))
    }
    ...
    pub fn read_bytes(&mut self, output: &mut [u8], delay: &mut impl DelayNs) -> OneWireResult<(), E> {
    ...
    pub fn match_address(&mut self, address: &Address, delay: &mut impl DelayNs) -> OneWireResult<(), E> {
    ...
    pub fn devices<'a, 'b, D>(&'a mut self, only_alarming: bool, delay: &'b mut D) -> DeviceSearch<'a, 'b, T, D>
    where
        D: DelayNs,
    {
        DeviceSearch {
    ...
    pub fn device_search(
        ...
        if let Some(search_state) = search_state {
            ...
            for bit_index in 0..search_state.last_discrepancy_index {
                ...
                let was_discrepancy_bit = (search_state.discrepancies & (1_u64 << (bit_index as u64))) != 0;
            ...
            discrepancies = search_state.discrepancies & !(1_u64 << (search_state.last_discrepancy_index as u64));
